	...
	for i = minDivisor; i < maxDivisor; i++ {
		go func(i uint32) {
			...
			if RoundPlus(1.0/float64(exactCount)*float64(fastCount), 4) != 1.0 {
				fmt.Println("Divisor:", i, "Zeroremainder counts:", fastCount, "Modulo operation counts:", exactCount, "Difference (%):", RoundPlus(100.0/float64(exactCount)*float64(fastCount), 4))
			}
		}(i)
	}
